...
public class MyDatabase {
	...
	public static void main(String[] args) {
		...
		try {
			...
			File idf = new File("id.ndx");			
			if(!idf.exists()){
				idf.delete();
			}
			File dlf = new File("last_name.ndx");			
			if(!dlf.exists()){
				dlf.delete();
			}
			File dsf = new File("state.ndx");			
			if(!dsf.exists()){
				dsf.delete();
			}
			...
			MapStringList<Integer, Long> idMap = new MapStringList<Integer, Long>();
			MapStringList<String, Long> lnameMap = new MapStringList<String, Long>();
			MapStringList<String, Long> stateMap = new MapStringList<String, Long>();
			...
			case "delete":
				...
				if (emp1.getState() != null) {
					...
					if(stateMap.getValues(emp1.getState().toLowerCase()).isEmpty()){
						stateMap.remove(emp1.getState().toLowerCase());
					}
				} 
				if (emp1.getLastName() != null) {
					...
					if(lnameMap.getValues(emp1.getLastName().toLowerCase()).isEmpty()){
						lnameMap.remove(emp1.getLastName().toLowerCase());
					}
			...
			case "insert":
				...
				long offsetnew = dbfile.getFilePointer();
				if (idMap.containsKey(Integer.parseInt(arr[0].substring(2)))) {
				...
				if (!idMap.map.containsKey(Integer.parseInt(args[1]))) {
					...
					if (oldlname != null) {
						...
						if(lnameMap.getValues(oldlname.toLowerCase()).isEmpty()){
							lnameMap.remove(oldlname.toLowerCase());
						}
						...
						if(stateMap.getValues(oldstate.toLowerCase()).isEmpty()){
							stateMap.remove(oldstate.toLowerCase());
						}
					}
